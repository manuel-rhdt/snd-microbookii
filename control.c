 ...
 */
static void microbookii_control_phono_mic_sw_update(struct microbookii_control *ctrl)
{
...
static int microbookii_control_phono_mic_sw_info(struct snd_kcontrol *kcontrol,
										  struct snd_ctl_elem_info *uinfo)
{
	return snd_ctl_enum_info(uinfo, 1, 2, phono_mic_sw_texts);
}

static int microbookii_control_phono_mic_sw_put(struct snd_kcontrol *kcontrol,
										 struct snd_ctl_elem_value *ucontrol)
{
	struct microbookii_control *ctrl = snd_kcontrol_chip(kcontrol);
	int changed = 0;

	if (ctrl->phono_mic_switch != ucontrol->value.enumerated.item[0]) {
		ctrl->phono_mic_switch = ucontrol->value.enumerated.item[0];

		microbookii_control_phono_mic_sw_update(ctrl);
...
static int microbookii_control_phono_mic_sw_get(struct snd_kcontrol *kcontrol,
										 struct snd_ctl_elem_value *ucontrol)
{
	struct microbookii_control *ctrl = snd_kcontrol_chip(kcontrol);
...
static struct snd_kcontrol_new elements[] = {
	{
		...
		.info = microbookii_control_phono_mic_sw_info,
		.get = microbookii_control_phono_mic_sw_get,
		.put = microbookii_control_phono_mic_sw_put
	},
	{}
};
